...
class PersistentStorageServertest(unittest.TestCase):
    ...
    def testGoodPatchRequests(self):
        ...
        self.conn.request('PATCH', 'C/user/2015-04-23T12:00:00Z/house2/bedroom/light1', 'CACTION1')        
        resp = self.conn.getresponse()
        self.assertEqual(resp.status, 200)
        ...
        resp = self.conn.getresponse()
        self.assertEqual(resp.status, 200)
        self.assertEqual(resp.read(), '[{"house_id": 50, "room_id": 50, "blob": "ACTION1", "time": 2014, "user-id": 50, "device_id": 123}, {"house_id": 50, "room_id": 50, "blob": "ACTION2", "time": 2015, "user-id": 50, "device_id": 123}]')
